...
void Ball::collide(double itemx, double itemy, double itemSpeedx, double itemSpeedy )
{
    ...
    dotProduct = (speedx - (-speedx))*dx + (speedy - (-speedy))*dy;
    speedCalculationx = speedx - dotProduct/pow(sqrt(pow(dx,2)+pow(dy,2)),2) * dx;
    speedCalculationy = speedy - dotProduct/pow(sqrt(pow(dx,2)+pow(dy,2)),2) * dy;
    setSpeed(speedCalculationx, speedCalculationy);
